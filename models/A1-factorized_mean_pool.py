"""
A1 -- Factorized attention, mean-pool readout

Factorized (axial) attention: the patches of one observation attend to one
another, then each patch looks back along its own history. The prediction at
each observation is read out by averaging that observation's patch tokens with
equal weights.
...
# --------------------------------------------------------------------------
# 3. Model -- Factorized attention, mean-pool readout
# --------------------------------------------------------------------------
# Each layer applies two attentions in sequence:
#   spatial   within one observation, all P patches attend freely (they share a
#             timestamp, so no mask is needed)
#   temporal  for each patch position independently, across observations, with a
#             causal mask (a token at t sees only t' <= t)
# Repeating this N_LAYERS times costs O(T*P^2 + P*T^2) per layer, against
# O((T*P)^2) for attention over all space-time tokens jointly.
# Readout: mean over the current observation's patch tokens.

class AxialFactorizedTransformer(nn.Module):
    def __init__(self, d_model, patch_size, H_pad, W_pad, Nch,
        ...
        self.patch_embed = nn.Conv2d(64, d_model,
                                     kernel_size=patch_size, stride=patch_size)

        self.spatial_pos = nn.Parameter(torch.randn(1, 1, self.P, d_model) * 0.02)
        self.time_embed  = Time2Vec(d_model)

        self.band_scale = nn.Embedding(Nch, d_model)
        self.band_shift = nn.Embedding(Nch, d_model)
        nn.init.ones_(self.band_scale.weight)
        nn.init.zeros_(self.band_shift.weight)
        ...
        self.spatial_layers  = nn.ModuleList([mk_layer() for _ in range(n_layers)])
        self.temporal_layers = nn.ModuleList([mk_layer() for _ in range(n_layers)])
    ...
    def _temporal_causal_mask(self, T, device):
        m = torch.triu(torch.ones(T, T, device=device), diagonal=1).bool()
        am = torch.zeros(T, T, device=device)
        am[m] = float('-inf')
        return am

    def forward(self, imgs, timestamps, band_ids):
        B, T, C, H, W = imgs.shape
        P, d = self.P, self.d_model

        flat = imgs.reshape(B * T, 1, H, W)
        x = self.cnn_stem(flat)
        x = self.patch_embed(x)
        x = x.flatten(2).transpose(1, 2).reshape(B, T, P, d)

        x = x + self.spatial_pos
        scale = self.band_scale(band_ids).unsqueeze(2)
        shift = self.band_shift(band_ids).unsqueeze(2)
        x = x * scale + shift
        t_emb = self.time_embed(timestamps.unsqueeze(-1))
        x = x + t_emb.unsqueeze(2)                          # (B, T, P, d)

        tmask = self._temporal_causal_mask(T, imgs.device)

        for sl, tl in zip(self.spatial_layers, self.temporal_layers):
            # spatial: within one observation, over the P patches
            xs = x.reshape(B * T, P, d)
            xs = sl(xs)                                     # no mask: same time
            x  = xs.reshape(B, T, P, d)

            # temporal: across observations, per patch position, causal
            xt = x.permute(0, 2, 1, 3).reshape(B * P, T, d)
            xt = tl(xt, src_mask=tmask, is_causal=False)
            x  = xt.reshape(B, P, T, d).permute(0, 2, 1, 3)

        h = x.mean(dim=2)                                   # (B, T, d) pool patches
...
if __name__ == "__main__":
    ...
    model = AxialFactorizedTransformer(
        d_model         = d_model,
        patch_size      = patch_size,
        H_pad           = H_pad,
        W_pad           = W_pad,
