"""
B3 -- Joint pyramid (fine windowed then coarse global)

Windowed attention on the fine 9x9 patch grid, then average-pooled to a coarse
grid where attention is unrestricted in space while remaining causal in time.
The readout is time-conditioned: the current observation's time embedding is
used as the query.
...
# --------------------------------------------------------------------------
# 3. Model -- Joint pyramid (fine windowed then coarse global)
# --------------------------------------------------------------------------
# Two stages per forward pass:
#   fine    windowed joint attention over the T*Pf fine patch tokens, causal in
#           time, keeping spatial precision where it is cheap
#   coarse  the fine grid is average-pooled by POOL to Pc tokens per observation,
#           over which attention is unrestricted in space and still causal in
#           time, so long-range mixing is recovered at low cost
# Readout: cross-attention over the coarse field, queried by the current
# observation's time embedding.
...
class PyramidJointTransformer(nn.Module):
    def __init__(self, d_model, patch_size, H_pad, W_pad, Nch,
                 n_heads=4, dim_feedforward=256, window_r=1, pool=3,
                 n_fine=2, n_coarse=2, readout="xattn"):
        super().__init__()
        self.d_model = d_model
        self.Ph = H_pad // patch_size; self.Pw = W_pad // patch_size
        self.Pf = self.Ph * self.Pw
        self.pool = pool
        self.Phc = self.Ph // pool; self.Pwc = self.Pw // pool
        self.Pc = self.Phc * self.Pwc
        self.window_r = window_r
        self.readout = readout
        # fine-grid row/col, used for the window mask
        pid = torch.arange(self.Pf)
        self.register_buffer("pf_row", pid // self.Pw)
        self.register_buffer("pf_col", pid %  self.Pw)
        ...
        self.spatial_pos = nn.Parameter(torch.randn(1, 1, self.Pf, d_model) * 0.02)
        self.coarse_pos = nn.Parameter(torch.randn(1, 1, self.Pc, d_model) * 0.02)
        ...
        self.fine_blocks = nn.ModuleList([EncLayer(d_model, n_heads, dim_feedforward) for _ in range(n_fine)])
        self.coarse_blocks = nn.ModuleList([EncLayer(d_model, n_heads, dim_feedforward) for _ in range(n_coarse)])
        if readout == "xattn":
    ...
    def _tp_mask(self, T, P, device, windowed):
        idx = torch.arange(T * P, device=device)
        ti = idx // P; pi = idx % P
        future = ti.unsqueeze(1) < ti.unsqueeze(0)
        if windowed:
            ri = self.pf_row.to(device)[pi]; ci = self.pf_col.to(device)[pi]
            dr = (ri.unsqueeze(1) - ri.unsqueeze(0)).abs()
            dc = (ci.unsqueeze(1) - ci.unsqueeze(0)).abs()
            allow = (~future) & (dr <= self.window_r) & (dc <= self.window_r)
        else:
            allow = ~future
        return torch.zeros(T * P, T * P, device=device).masked_fill(~allow, -1e9)

    def _read_mask(self, T, P, device):
        qi = torch.arange(T, device=device).unsqueeze(1)
        kt = (torch.arange(T * P, device=device) // P).unsqueeze(0)
        return torch.zeros(T, T * P, device=device).masked_fill(kt > qi, -1e9)

    def forward(self, imgs, timestamps, band_ids):
        B, T, C, H, W = imgs.shape; d = self.d_model
        x = self.cnn_stem(imgs.reshape(B * T, 1, H, W))
        x = self.patch_embed(x).flatten(2).transpose(1, 2).reshape(B, T, self.Pf, d)
        ...
        x = x + t_emb.unsqueeze(2)                            # (B,T,Pf,d)
        # fine stage: windowed joint attention
        win_m = self._tp_mask(T, self.Pf, imgs.device, True)
        xf = x.reshape(B, T * self.Pf, d)
        for blk in self.fine_blocks:
            xf = blk(xf, win_m)
        x = xf.reshape(B, T, self.Pf, d)
        # downsample: spatial average-pool the fine grid
        xg = x.reshape(B * T, self.Ph, self.Pw, d).permute(0, 3, 1, 2)        # (B*T,d,Ph,Pw)
        xg = F.avg_pool2d(xg, kernel_size=self.pool, stride=self.pool)         # (B*T,d,Phc,Pwc)
        xc = xg.flatten(2).transpose(1, 2).reshape(B, T, self.Pc, d)          # (B,T,Pc,d)
        xc = xc + self.coarse_pos + t_emb.unsqueeze(2)
        # coarse stage: globally-connected joint attention
        full_m = self._tp_mask(T, self.Pc, imgs.device, False)
        xcf = xc.reshape(B, T * self.Pc, d)
        for blk in self.coarse_blocks:
            xcf = blk(xcf, full_m)
        xc = xcf.reshape(B, T, self.Pc, d)
        # causal readout
        if self.readout == "xattn":
            kv = xc.reshape(B, T * self.Pc, d)
            q = t_emb                                          # (B,T,d) per-timestep query
            h, _ = self.read_attn(q, kv, kv, attn_mask=self._read_mask(T, self.Pc, imgs.device), need_weights=False)
            h = self.read_norm(h)
        else:
            h = xc.mean(dim=2)
