"""
A2 -- Factorized attention, learned-query readout

Backbone as A1. The readout is a causal cross-attention pool: one learned
query per observation gathers all causally available tokens with learned
weights, so the readout can select what it needs as the sequence evolves.
...
# --------------------------------------------------------------------------
# 3. Model -- Factorized attention, learned-query readout
# --------------------------------------------------------------------------
# Backbone as A1. The readout replaces the mean pool: one learned query per
# observation, specialised by that observation's time embedding, attends over
# every token (t', p) with t' <= t and pools them into the classification
# vector. The weighting is learned, so the readout can select what it needs as
# the sequence grows.

class CrossAttnPoolAxialTransformer(nn.Module):
    def __init__(self, d_model, patch_size, H_pad, W_pad, Nch,
        ...
        # readout query: one learned base query, specialised by the time embedding
    ...
    def _read_mask(self, T, P, device):
        # (T, T*P): query t attends key (t', p) iff t' <= t
        qi = torch.arange(T, device=device).unsqueeze(1)            # (T,1)
        kt = (torch.arange(T * P, device=device) // P).unsqueeze(0) # (1,T*P)
        allow = kt <= qi
    ...
    def forward(self, imgs, timestamps, band_ids):
        ...
        t_emb = self.time_embed(timestamps.unsqueeze(-1))           # (B,T,d)
        ...
        # causal cross-attention readout
        kv = x.reshape(B, T * P, d)                                 # keys/values
        q = self.read_q + t_emb                                     # (B,T,d) per-timestep query
        rmask = self._read_mask(T, P, imgs.device)                  # (T, T*P)
        h, _ = self.read_attn(q, kv, kv, attn_mask=rmask, need_weights=False)
        h = self.read_norm(h)
...
if __name__ == "__main__":
    ...
    model = CrossAttnPoolAxialTransformer(
        d_model         = d_model,
        patch_size      = patch_size,
        H_pad           = H_pad,
        W_pad           = W_pad,
