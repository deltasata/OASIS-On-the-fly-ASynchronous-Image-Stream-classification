"""
B1 -- Joint global space-time attention

One causal attention over all N_obs * N_P = 1134 space-time tokens: every
visible token can attend to every other visible token. Mean-pool readout.
...
# --------------------------------------------------------------------------
# 3. Model -- Joint global space-time attention
# --------------------------------------------------------------------------
# One attention over all T*P space-time tokens with a causal block mask: token
# (t, p) attends to every token (t', p') with t' <= t. Within an observation all
# patches attend freely, and every earlier observation is fully visible.
# Readout: mean over the current observation's patch tokens.

class JointSpacetimeTransformer(nn.Module):
    def __init__(self, d_model, patch_size, H_pad, W_pad, Nch,
                 n_heads=4, n_layers=4, dim_feedforward=256, readout="mean"):
        super().__init__()
    ...
    def _build_causal_mask(self, T, device):
        # (T*P, T*P): block a key whose observation is later than the query's.
        # Within an observation all patches attend freely; every earlier
        # observation is fully visible.
        P = self.P
        t_idx = torch.arange(T * P, device=device) // P
        mask = t_idx.unsqueeze(0) > t_idx.unsqueeze(1)
        am = torch.zeros(T * P, T * P, device=device); am[mask] = float('-inf')
        return am
    ...
    def forward(self, imgs, timestamps, band_ids):
        ...
        x = x.reshape(B, T * P, d)
        attn_mask = self._build_causal_mask(T, imgs.device)
        h = self.transformer(x, mask=attn_mask, is_causal=False)   # (B, T*P, d)
        if self.readout == "xattn":
            kv = h
            q = self.read_q + t_emb
            rmask = self._read_mask(T, P, imgs.device)
            r, _ = self.read_attn(q, kv, kv, attn_mask=rmask, need_weights=False)
            h = self.read_norm(r)                                   # (B, T, d)
        else:
            h = h.reshape(B, T, P, d).mean(dim=2)                  # (B, T, d)
...
if __name__ == "__main__":
    ...
    model = JointSpacetimeTransformer(
        d_model         = d_model,
        patch_size      = patch_size,
        H_pad           = H_pad,
        W_pad           = W_pad,
